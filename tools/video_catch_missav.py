...
# ------------------ 导入项目模块 ------------------
from tools.catch import run_curl, config
...
def parse_page(html):
    ...
    return title, actress_name, poster_url, m3u8_url, tag_list, download_path
...
def main():
    ...
    # 从 config 中获取 的 URL
    entry = next(
        (d for d in config['DataSources'] if d.get('name') == 'MissAV'),
        None
    )
    ...
    html = run_curl(url)
    title, actress_name, poster_url, m3u8_url, tag_list, download_path = parse_page(html)
    ...
    # 打印结果
    print(f"title_missav: {title}") 
    print(f"actress_name_missav: {actress_name}")
    print(f"poster_url_missav: {poster_url}") 
    print(f"m3u8_url_missav: {m3u8_url}") 
    print(f"Tags_missav: {tag_list}") 
...
#         print(f"actress_name_missav: {actress_name}")
#         print(f"poster_url_missav: {poster_url}") 
#         print(f"m3u8_ur_missav: {m3u8_url}") 
#         print(f"Tags_missav: {tag_list}") 
#         print(f"download_path: {download_path}") 
